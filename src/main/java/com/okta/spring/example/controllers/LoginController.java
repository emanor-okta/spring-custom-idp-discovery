...
public class LoginController {
    ...
    @GetMapping(value = "/discovery-callback")
    public ModelAndView discoveryCallback(HttpServletRequest request, HttpServletResponse response,
                              @RequestParam(name = "fromLogin", required=false) String fromLogin) throws MalformedURLException {

        if (fromLogin != null && fromLogin.equalsIgnoreCase("true")) {
            Cookie cookie = new Cookie("fromLogin", "true");
            cookie.setMaxAge(5);       // 5 seconds
            response.addCookie(cookie);
            return new ModelAndView("redirect:" + "http://localhost:8080/oauth2/authorization/okta");
        }
        ...
        for (Cookie c : request.getCookies()) {
            if (c.getName().equalsIgnoreCase("fromLogin") && c.getValue().equalsIgnoreCase("true")) {
                return new ModelAndView("redirect:" + oktaOAuth2Properties.getIssuer() +
                        ...
                        "&state=" + state +
                        "&redirect_uri=http://localhost:8080" + oktaOAuth2Properties.getRedirectUri() +
                        "&nonce=" + nonce);
            }
        }
